...
def video_detect(request):  # 视频检测
    ...
    if request.method == "POST":
        ...
        while i < len(imagesPaths):  # 对图片进行循环
            if not saveTxt(imagesPaths[i], darknet.detect(net, meta, imagesPaths[i]), times[i], trashCanA, trashCanB):  # 判断是否检测到垃圾桶
                count = count + 1
                begin = False
            else:
                begin = True
            ...
            if (not begin) and count > 5:  # 连续5次检测不到，进行跳跃式检测
                index = i
                while index + 10 < len(imagesPaths) and len(darknet.detect(net, meta, imagesPaths[index + 10])) < 1:  # 每隔一秒判断一次,寻找下一个起始点
                    index = index + 10
                i = index
                count = 0
            i = i + 1
...
def getImgsPath(videoPath):  # 视频提取帧
    ...
    delFile(filePath + '/MyCache/videoCache')  # 清空缓存
    ...
    while success:  # 循环读取图片
        ...
        if success and frame is not None and frame_count % per_count == 0:  # 每3帧取一帧图片保存下来，可以自己修改
            ...
            if minutes >= 60:
                hours = minutes // 60
                minutes = minutes % 60
            ss = str(int(hours)) + "-" + str(int(minutes)) + "-" + str(int(seconds)) + "-" + str(int(milliseconds))  # 将时间写成字符串，用于命名
            cv2.imwrite(filePath + '/MyCache/videoCache/%s.jpg' % ss, frame)  # 保存为图片
            paths.append(filePath + '/MyCache/videoCache/%s.jpg' % ss)  # 添加路径
...
def base64_to_img(str):  # 将base64码转换成图片
    ...
    filePath = filePath.replace("\\", '/')
    curtime = time.localtime(time.time())  # 获取当前时间
    with open(filePath + '/MyCache/cameraCache/%s.jpg' % (
            time.strftime('%Y-%m-%d-%H-%M-%S', curtime)),
              'wb+') as f:  # 写入图片
        f.write(image_data)
    return filePath + '/MyCache/cameraCache/%s.jpg' % (time.strftime('%Y-%m-%d-%H-%M-%S', curtime)), curtime  # 返回时间与图片路径
...
# 模拟测试摄像头输入
imagePaths, curTimes = getImgsPath('/home/lxp/桌面/video/ccccccc11111.mp4')  # 视频处理为图片
...
def camera_detect(request):  # 摄像头输入
    ...
    valueA = []
    valueB = []
    global cameraTrashCanA, cameraTrashCanB, cameraCurTimes, cameraImgPaths, cameraBeginTimeA, cameraBeginTimeB, imgCount, cameraBeginTime, imagePaths  # 申明调用全局变量
    if request.method == "POST":
        photoPath = request.POST.get('Path')
        imgPath, curTime = base64_to_img(photoPath)  # 获取图片信息
        if imgCount < len(imagePaths):
            cameraImgPaths.append(imagePaths[imgCount])
            cameraCurTimes.append(curTimes[imgCount])
            imgCount = imgCount + 1
        if curTimes[imgCount - 1] - cameraBeginTime > 10000:  # 当持续时间超过10s时，默认图片够多
            cameraBeginTime = curTimes[imgCount - 1]
            count = 0
            i = 0
            while i < len(cameraImgPaths):  # 循环
                ...
                if (not begin) and count > 5:  # 连续5次美检测到， 进行跳跃式检测
                    ...
                    count = 0
                i = i + 1
            cameraCapacityA, cameraCapacityB = GetCapacity.getCapacity(cameraTrashCanA, cameraBeginTimeA, curTimes[imgCount - 1],
                                                                       cameraTrashCanB, cameraBeginTimeB, curTimes[imgCount - 1])  # 获取容量
    ...
    return HttpResponse(json.dumps({  # http返回给ajax，用于动态补点
        "timeA": json.dumps(timeA),
        "valueA": json.dumps(valueA),
        "timeB": json.dumps(timeB),
        "valueB": json.dumps(valueB)
